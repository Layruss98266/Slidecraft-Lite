"""
SlideCraft Lite — Logo + Watermark Removal (low-RAM build)
...
# Hard 10 MB cap on every export — iteratively re-rendered to fit.
...
def remove_edstellar_text():
    ...
    ocr_unavailable = 0  # slides we would have OCR'd but cannot
    for i, path in enumerate(slides, start=1):
        ...
        h, w = img.shape[:2]
        hits = []
        pdf_regions = pdf_cache.get(str(i)) or []
    ...
    # ─── Pass 2: inpaint + write only the slides with hits
    removed = 0
    touched_slides = 0
    for e in per_slide:
        if not e["hits"]:
    ...
    resp = {"ok": True,
            ...
            "log_id": log_id}
    if ocr_unavailable:
        resp["skipped"] = "ocr-unavailable"
        resp["ocr_unavailable_slides"] = ocr_unavailable
    return jsonify(resp)
